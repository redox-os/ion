    ...
    where I::Item: AsRef<str>
{
    match parse_assignment(args) {
        ...
        Binding::Math(key, operator, increment) => {
            let _ = match vars.get_var_or_empty(&key).parse::<f32>() {
                ...
                Err(_) => {
                    ...
                    let _ = writeln!(stderr, "ion: original value, {}, is not a number", key);
                    return FAILURE;
                }
            };
        },
    ...
    where I::Item: AsRef<str>
{
    match parse_assignment(args) {
        ...
        Binding::Math(key, operator, increment) => {
            match vars.get_var(&key).unwrap_or_else(|| "".to_owned()).parse::<f32>() {
                Ok(old_value) => match operator {
